...
function rerenderRight(){
  ...
  else{const [id,kind]=String(sel).split('|');const e=EQMAP[+id]; if(e)renderEqDetail(e,kind);}
}
$('list').addEventListener('click',ev=>{
  const el=ev.target.closest('.row'); if(!el)return;
  if(TAB==='mat'){sel=+el.dataset.id;const m=DB.materials.find(x=>x.id===sel);renderMatList();renderMatDetail(m);}
